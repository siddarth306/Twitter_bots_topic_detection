...
def get_preprocessed_data_coronavirus(tool_config):
    ...
    if len(country_df.index) > 0:
        country_df = preprocessing.tweets_cleaning_coronavirus(country_df,
                                                   nphrases,
                                                   phrases_country,
                                                   phrases_country_freq,
                                                   phrases_hashtag,
...
def save_data_coronavirus(data_df, en, tool_config, country):
    ...
    tweet_df[["tid", "uid"]] = tweet_df.apply(lambda row: pd.Series(convert_tids_uids(row["tid"], row["uid"])), axis=1)
        ...
        for url in urls:
            try:
                ...
                if unique_links_map.get(url, None) is None:
                    unique_links_map[url] = [url, set([row["uid"]]), row["share_count"]+1, row["likes_count"], row["timestamp"]]
                else:
    ...
    clusters_list.append([["corona virus", "covid 19"],["corona-virus"], likes, shares, bot_count, tweet_number, top_tweets_list, top_links_list])
...
def calculate_clusters_coronavirus(tool_config):
    ...
    try:
        ...
        all_country_en_dfs = get_preprocessed_data_coronavirus(tool_config)
    ...
    for key, val_df in all_country_en_dfs.items():
        if len(val_df) > 0:
            save_data_coronavirus(val_df, True, tool_config, key)
